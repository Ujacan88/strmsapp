...
// KPI stavy — tržba sa počíta len pre záväzné stavy
const KPI_STAVY = new Set(['Objednané','Realizované']);

// calcKpi — pre dashboard súčty (len záväzné stavy)
function calc(rows) {
    return rows.map(r => {
...
// calcFull — pre zobrazenie zákazky (vždy počíta, aby videli hodnotu)
...
router.get('/', (req, res) => {
    ...
    // recent 10 zakazky
    const recent = calcFull(query(`SELECT * FROM zakazky ${wZ} ORDER BY created_at DESC LIMIT 5`, p));
