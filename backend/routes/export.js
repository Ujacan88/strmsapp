...
function buildWhere(req, type) {
  ...
  if (type==='zakazky') {
    const { stav, typ_prod, zdroj, q, rok, mesiac } = req.query;
    ...
    if (rok && mesiac) { parts.push('created_at LIKE ?'); params.push(`${rok}-${mesiac}%`); }
    else if (rok)      { parts.push('created_at LIKE ?'); params.push(`${rok}-%`); }
    else if (mesiac)   { parts.push('created_at LIKE ?'); params.push(`%-${mesiac}-%`); }
  } else {
    const { typ, zaruka, q, rok, mesiac } = req.query;
    ...
    if (rok && mesiac) { parts.push('datum LIKE ?'); params.push(`${rok}-${mesiac}%`); }
    else if (rok)      { parts.push('datum LIKE ?'); params.push(`${rok}-%`); }
    else if (mesiac)   { parts.push('datum LIKE ?'); params.push(`%-${mesiac}-%`); }
  }
