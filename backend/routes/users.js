...
router.delete('/:id', (req, res) => {
    ...
    const target = require('../db').queryOne('SELECT username, role FROM users WHERE id = ?', [req.params.id]);
    if (target && target.username === 'adm.herstekmatej') return res.status(400).json({ error: 'Systémový účet nie je možné vymazať.' });
    if (target && target.role === 'admin' && req.user.role === 'owner') return res.status(400).json({ error: 'Konateľ nemôže odstrániť admin účty.' });
